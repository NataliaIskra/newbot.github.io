...
# --- Конфигурация ---
# Читаем токен и ID админа из переменных окружения
BOT_TOKEN = os.environ.get('BOT_TOKEN')
ADMIN_CHAT_ID = os.environ.get('ADMIN_CHAT_ID')

# Проверка, что переменные были найдены
...
def analyze_results(user_id):
    ...
    # Шаг 1: Определяем, какой вердикт вынести
    ...
    # Шаг 2: Получаем всю информацию о вердикте из одного места
    verdict_info = VERDICT_DATA[verdict_key]

    # Шаг 3: Отправляем полный отчет админу
    notify_admin(user_id, data, verdict_info['name'], verdict_info['text'])

    # Шаг 4: Отправляем вердикт пользователю
...
if __name__ == '__main__':
    print("Запускаю веб-сервер для поддержки активности...")
    keep_alive()  # Эта строка запустит веб-сервер в фоновом режиме
    print("Бот запущен...")
    try:
        bot.polling(none_stop=True)
    except Exception as e:
        print(f"Бот упал с ошибкой: {e}")
        # Можно добавить перезапуск или логирование